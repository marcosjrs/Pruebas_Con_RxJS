...
rxjs
    .fromEvent(boton, 'click')
    .pipe(
        rxjs.operators.throttleTime(1000), //hasta que pase un segundo no ejecutará lo siguiente
        rxjs.operators.scan((veces) => veces + 1, 0)
...
rxjs
    ...
    .pipe(
        rxjs.operators.throttleTime(1000), //hasta que pase un segundo no ejecutará lo siguiente
        rxjs.operators.map((evento) => evento.clientX),//map recibe el valor anterior, en este caso es el evento click, y de ese evento pasa el valor de ClientX al siguiente paso del flujo
        rxjs.operators.scan((acumulador, valorDelMap) => acumulador + valorDelMap, 0) //acumulador es como el valor que se va acumulando, comenzamos con valor 0. El segundo parametro es el valor pasado por el anterior del flujo. Es un reducer.
        ) //El valor del scan se le pasará al subscribe, no tiene porque tener el mismo nombre, pero la variable es del ambito del scan.
